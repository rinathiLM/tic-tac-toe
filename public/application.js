...
var click = function click(event) {
  $('#logged-in-message').text('');
  event.preventDefault();
  var cell = event.target.attributes[1].value;
  // console.log(cell)
  moves += 1;
  ...
  if (moves < 10) {
    ...
    api.updateTurn(cell, currentPlayer).then(ui.updateSuccess).catch(ui.updateFailure);
    if (grid[0] === grid[1] && grid[0] === grid[2]) {
      ...
      $('.cell').off('click');
      api.updateDone(cell, currentPlayer).then(ui.updateSuccess).catch(ui.updateFailure);
    }
  }
...
var gameOver = function gameOver(player) {
  ...
  api.updateDone().then(ui.updateSuccess).catch(ui.updateFailure);
};
...
var onSignOut = function onSignOut(event) {
  ...
  authApi.signOut().then(authUi.signOutSuccess).catch(authUi.signInFailure);
};
...
var updateTurn = function updateTurn(cell, currentPlayer) {
  return $.ajax({
...
var updateDone = function updateDone(cell, currentPlayer) {
  return $.ajax({
    url: config.apiOrigin + '/games/' + game.game.id,
    method: 'PATCH',
    headers: {
...
var createSuccess = function createSuccess(gameObject) {
...
var getStatsSuccess = function getStatsSuccess(data) {
  // console.log(data)
  var gameTotal = data.games.length;
